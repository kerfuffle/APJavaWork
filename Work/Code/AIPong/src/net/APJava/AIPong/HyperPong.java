...
/* Version 2-10-26-15--This version has less choppy animation due to forcing the
 ...
 class AlgorithmicPlayer extends Player {
	 ...
	 void update(GameInterface game) {
		 ...
		 {
			 if (ballX < HyperPong.WIDTH/2+75 && whichPlayer == 1)
			 {
			 ...
			 if (ballX > HyperPong.WIDTH/2-75 && whichPlayer == 2)
			 {
				 if (ourCenterY < ballY+(ball.getH()/2))
				 {
					 moveDown();
	 ...
	 private boolean isWithin(int pY, int pH, int bY, int bH)
	 {
		 if ((bY > pY && bY < pY + pH ) || (bY + bH > pY && bY+bH < pY+pH))
		 {
			 return true;
		 }
		 return false;
